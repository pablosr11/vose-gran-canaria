...
async function scrapeCinema(url, name, type) {
  ...
  try {
    ...
    try {
      const selectors = ['#onetrust-accept-btn-handler', '#accept-cookies', '.cc-accept', '.accept-all', 'button:contains("Aceptar")'];
    ...
    const results = [];
    const voseRegex = /VOSE|V\.O\.|Original Subtitulada|Versión Original|SUBTITULADA/i;
    ...
    if (type === 'ocine') {
      $('article, [class*="movie"], .swiper-slide, .card').each((i, el) => {
        ...
        if (voseRegex.test(text)) {
          ...
          if (link) {
            movieUrl = link.startsWith('http') ? link : new URL(link, url).href;
          }
